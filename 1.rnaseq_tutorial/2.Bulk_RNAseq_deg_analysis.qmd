...
RNA-seq data sets have 1000s of genes, but we don't want to individually run 1000s of statistical models! In this section, we introduce some R packages designed for running linear and mixed effects models across all genes in an efficient manor.
...
Specifically, we will be focusing on the T-cell transcriptional responses to *M. tuberculosis* (Mtb) challenge at week 25 after BCG vaccination. These data are pseudo-bulk counts from the larger single-cell data set, which we will be exploring later on.
...
You can then incorporate this estimate into the `limma` model. We will not do not here, because we now have a true mixed effects tools in `kimma`.
